...
export function convertZohoContactToContact(
  zohoContact: IZohoContact
): Contact | null {
  if (!zohoContact.id) {
    return null;
  }
...
function collectPhoneNumbersFromZohoContact(
  zohoContact: IZohoContact
): PhoneNumber[] {
  ...
  if (zohoContact.Home_Phone) {
    phoneNumbers.push({
      ...
      phoneNumber: zohoContact.Home_Phone
    });
  }
  if (zohoContact.Phone) {
    phoneNumbers.push({
      label: PhoneNumberLabel.WORK,
      phoneNumber: zohoContact.Phone
    });
  }
  if (zohoContact.Mobile) {
    phoneNumbers.push({
      label: PhoneNumberLabel.MOBILE,
      phoneNumber: zohoContact.Mobile
    });
  }
  if (zohoContact.Fax) {
    phoneNumbers.push({
      label: "FAX" as PhoneNumberLabel,
      phoneNumber: zohoContact.Fax
    });
  }
  if (zohoContact.Other_Phone) {
    phoneNumbers.push({
      label: "OTHER" as PhoneNumberLabel,
      phoneNumber: zohoContact.Other_Phone
    });
  }
...
function tryConvertNameToFirstLastName(contact: ContactTemplate) {
  ...
  const nameAttrs = humanparser.parseName(contact.name);
  contact.firstName =
    nameAttrs.firstName + (nameAttrs.middleName ? nameAttrs.middleName : "");
  contact.lastName = nameAttrs.lastName;
}

export function convertContactToZohoContact(
